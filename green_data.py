...
def get_tracts():
    ...
    """
    tracts_url = 'https://opendata.arcgis.com/datasets/de58dc3e1efc49b782ab357e044ea20c_9.geojson'
...
def get_zips():
    ...
    """
    zips_url = 'https://opendata.arcgis.com/datasets/83fc2e72903343aabff6de8cb445b81c_2.geojson'
...
def get_gdf(year):
    '''
    Enter the desired year to download the traffic flow count data for that year. Example: enter '7' for the year 2007.
    '''
    num = year-7
...
def get_alldata():
    ...
    def get_racks():
        ...
        for zipcode in zip_list:
            indices = zips_racks_cleaned[zips_racks_cleaned.ZIPCODE==zipcode].index
            zips_racks_cleaned.loc[indices, 'RACK_CAPACITY'] = zips_racks_cleaned.loc[indices, 'RACK_CAPACITY'].cumsum()
    ...
    def get_lanes():
        ...
        l = gpd.read_file(bike_lanes_url)

        # Initial selection of relevant columns
        lane_columns = ['LENGTH_MILES', 'DATE_COMPLETED', 'geometry']
        bike_lanes = l[lane_columns]

        # Converts the date completed column to year
        bike_lanes['DATE_COMPLETED'] = pd.to_datetime(bike_lanes['DATE_COMPLETED'], unit='ms')
        bike_lanes['Year'] = pd.DatetimeIndex(bike_lanes['DATE_COMPLETED']).year
        ...
        zips_lanes_base = gpd.sjoin(get_zips(), bike_lanes_baseline, op='intersects')
        ...
        for zipcode in zip_list:
            selection = zips_lane_base.loc[zips_lane_base['ZIPCODE'] == zipcode]
            zip_sum = selection['LENGTH_MILES'].sum()
            zip_sum_list.append(zip_sum)
        base_df = pd.DataFrame(list(zip(zip_list, year_fill, zip_sum_list)), index=range(len(zip_list)), 
                               columns=['ZIPCODE', 'Year', 'Miles_Bike_Lanes'])
        '''
        ...
        zips_lanes = gpd.sjoin(get_zips(), bike_lanes_filtered, op='intersects')
        ...
        zips_lanes_cleaned.rename(columns={'LENGTH_MILES': 'Miles_Bike_Lanes'}, inplace=True)

        #zips_lanes_cleaned = base_df.append(zips_lanes_clean)
        ## MATTS FUNCTION GOES HERE ##

        # Inserts cumulative bike lane lengths over time
        zip_list = zips_lanes_cleaned.ZIPCODE.unique()
        for zipcode in zip_list:
            indices = zips_lanes_cleaned[zips_lanes_cleaned.ZIPCODE==zipcode].index
            zips_lanes_cleaned.loc[indices, 'Miles_Bike_Lanes'] = zips_lanes_cleaned.loc[indices, 'Miles_Bike_Lanes'].cumsum()
    ...
    def get_traffic(year):
        '''
        This function a GeoDataFrame of traffic volume by zip code for a given year.
        '''
        ...
        zip_mids_clean = zip_mids_clean.drop(columns=['SHAPE_Area','index_right','MIDPOINT'])
    ...
    # Creates dataframes for bike rack capacities, bike lane lengths, population and traffic volumes
    rack_data = get_racks()
    lane_data = get_lanes()
    pop_data = get_pop()
...
def get_zipdata(zipcode):
    '''
    This function a GeoDataFrame of traffic volume by year for a given zip code.
    '''
...
def get_csv(df, file_name):
    """
    This function takes an input DataFrame (df) and file name (file_name) and writes the DataFrame to a csv file titled file_name.
    """
    directory = os.getcwd()
...
def get_csv_alldata():
    """
    This function writes the DataFrame with all attributes by zip code and year to a csv file titled 'all_data.csv'.
    """
